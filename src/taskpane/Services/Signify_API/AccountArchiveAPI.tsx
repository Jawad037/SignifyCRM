...
import { getUserId } from './SignifyLoginAPI';
            ...
            if (result.status === Office.AsyncResultStatus.Succeeded) {
                ...
                fetch(`${API_URL}set_entry`, requestOptions)
                    ...
                    .then((result) => {
                        ...
                        const jsonAccountId = JSON.parse(oppOutput);
                        const AccountArchiveId = jsonAccountId.id;


                        getAccountArchiveID(AccountArchiveId)
...
export default AccountArchiveAPI

export const getAccountArchiveID = (AccountArchiveId) => {
    const API_URL = CRM_Url()
    const LoginToken = CRM_Token()
    const AccountID = getAccountId();
    const { API_Key, User_ID } = GetUserData()
